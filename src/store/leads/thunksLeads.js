...
import {
   createdNewLead_Netsuite,
   editarInformacionLead_Netsuite,
   get_optionLoss,
   get_Specific_Lead,
   getAll_LeadsRepit,
   getAllLeadsAttention,
   getAllLeadsComplete,
   getAllLeadsNew,
   getAllLeadsTotal,
   getAllStragglers,
   getBitacora,
   getDataInformations_Lead,
   getDataLead_Netsuite,
   getDataSelect_Admins,
   getDataSelect_Campaing,
...
export const generateLeadBitacora = (idnetsuite_admin, leadId, additionalValues, descripcionEvento, valueStatus) => {
   return async (dispatch) => {
      try {
         ...
         await insertBitcoraLead(
            leadId,
            idnetsuite_admin,
            additionalValues.valorDeCaida,
            descripcionEvento,
            additionalValues.tipo,
            estadoActual,
         );
   ...
   return async (dispatch, getState) => {
      ...
      try {
         ...
         await ensureLeadOpportunitiesAreInactive(leadId, {
            ...
            idnetsuite_admin,
            actorType: "USUARIO",
            source: "LEAD_PERDIDO_UI",
            detail: "Inactivación automática al marcar lead como perdido",
         });
         // Retorna "ok" si todo salió correctamente
         return "ok";
      } catch (error) {
...
export const createNoteFollow_up = (nota, leadId, selectedLossOption, followUpDate, leadStatus) => {
   return async (dispatch, getState) => {
      ...
      try {
         ...
         await dispatch(generateLeadBitacora(idnetsuite_admin, leadId, additionalValues, descripcionEvento, valueStatus));
         await ensureLeadOpportunitiesAreInactive(leadId, {
            reason: "LEAD_SEGUIMIENTO",
            idnetsuite_admin,
            actorType: "USUARIO",
            source: "LEAD_SEGUIMIENTO_UI",
            detail: `Inactivación automática al poner lead en seguimiento para ${followUpDate}`,
         });
...
/**
 * Action `editarInformacionLead`:
 * Esta función actualiza la información de un lead (cliente) en la base de datos.
 * Recibe la información actualizada (`formData`) como parámetro.
 *
 * @param {Object} formData - La información actualizada del lead.
 * @returns {function} - Una función asíncrona que realiza la operación de actualización.
 */
export const editarInformacionLead = (formData) => {
   return async () => {
      try {
         // Llama a la función editarInformacionLead_Netsuite para actualizar la información del lead en la base de datos.
         const result = await editarInformacionLead_Netsuite({ formData });

         // Devuelve los datos de las campañas obtenidos para su uso posterior.
         return result;
      } catch (error) {
         // En caso de error, muestra un mensaje en la consola y el detalle del error para facilitar el diagnóstico.
         console.error("Error al editar la informacion del lead", error);
      }
   };
};
